    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const res = await fetch("http://localhost:8000/auth/google", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: userInfo.email, name: userInfo.name, google_id: userInfo.sub })
        }).then(r => r.json());
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.card}>
        ...
        <p className={styles.sub}>
          {mode === "login" ? "Sign in to your session" : mode === "register" ? "Register to get started" : "Enter your email to receive a reset link"}
        </p>
        ...
        <p className={styles.switch}>
          ...
          <span className={styles.link} onClick={() => { setMode("login"); setError(""); setSuccess(""); }}>
            {mode === "forgot" ? "Sign In" : mode === "login" ? "" : ""}
          </span>
          {mode !== "forgot" && (
            <span className={styles.link} onClick={switchMode}>
              {mode === "login" ? "Register" : "Sign In"}
